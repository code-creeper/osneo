...
class LexofficeApi {
    ...
    public function __construct($settings = array()) {
        //if (!is_array($settings)) report('lexoffice-php-api: settings should be an array');
        //if (!array_key_exists('api_key', $settings)) report('lexoffice-php-api: no api_key is given');

        $this->api_key = config('lexoffice.api_key');
    ...
    protected function api_call($type, $resource, $uuid = '', $data = '', $params = '', $return_http_header = false, int $count = 1) {
        ...
        $url = "/$this->api_version/$resource/$uuid$params";
        Log::channel('lexoffice')->info(
            "$http_status $type $url \n".
            ($resource == 'files' ? null : $result)
        );
    ...
    public function get_pdf_file($uuid)
    {
        return $this->api_call('GET', 'files', $uuid, return_http_header: true);
    }

    public function save_pdf_by_uuid($uuid, $filename): bool {
        $request_file = $this->api_call('GET', 'files', $uuid);
        if ($request_file) {
            file_put_contents($filename, $request_file);
            return true;
        }
    ...
    public function get_needed_voucher_booking_id(float $taxrate, string $country_code, int $date, bool $euopean_vatid, bool $b2b_business, bool $physical_good = true): string {
        ...
        throw new LexofficeException('lexoffice-php-api: unknown booking scenario, cannot decide correct booking category', [
            'taxrate' => $taxrate,
            'country_code' => $country_code,
            'date' => $date,
            'european_vatid' => $euopean_vatid,
